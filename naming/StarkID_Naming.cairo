...
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    starkid_contract_addr, pricing_contract_addr, admin, l1_contract
) {
    ...
    starkid_contract.write(starkid_contract_addr);
...
func buy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (admin_address:felt) = _admin_address.read();
    pay_buy_domain(current_timestamp, days, caller, domain, sponsor, admin_address);
...
func buy_discounted{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (admin_address:felt) = _admin_address.read();
    pay_buy_domain_discount(current_timestamp, days, caller, domain, discount.amount,admin_address);
    mint_domain(expiry, resolver, address, hashed_domain, token_id, domain);
    return ();
}
...
func renew{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // Make the user pay
    let (admin_address:felt) = _admin_address.read();
    pay_renew_domain(current_timestamp, days, caller, domain, sponsor, admin_address);

    // Write info on stark.id and write info on storage data
    write_domain_data(1, new (domain), data);

    stark_id_update.emit(1, new (domain), domain_data.owner, expiry);
    return ();
}
...
func transfer_domain{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (current_timestamp) = get_block_timestamp();
    assert_empty_stark_id(target_token_id, current_timestamp, naming_contract);
    ...
    if (domain_len == 1) {
        ...
        StarkId.set_verifier_data(contract, current_domain_data.owner, 'name', 0);
        StarkId.set_verifier_data(contract, target_token_id, 'name', hashed_domain);
        return ();
    } else {
        let (hashed_parent_domain) = hash_domain(domain_len - 1, domain + 1);
