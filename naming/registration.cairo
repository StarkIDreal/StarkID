...
from src.interface.starkid import StarkId
...
@event
func stark_id_update(domain_len: felt, domain: felt*, owner: felt, expiry: felt) {
}
...
@storage_var
func starkid_contract() -> (address: felt) {
}
...
func pay_buy_domain{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    current_timestamp, days, caller, domain, sponsor,_admin_address
) -> () {
    ...
    with_attr error_message("ERC20 transfer impossible: check your ETH balance") {
        ...
        IERC20.transferFrom(erc20, caller, _admin_address, price);
    }
...
func pay_buy_domain_discount{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    current_timestamp, days, caller, domain, discount_amount,_admin_address
) -> () {
    alloc_locals;
    let (pricing_contract) = _pricing_contract.read();
    let (erc20, price) = Pricing.compute_buy_price(pricing_contract, domain, days);
    ...
    with_attr error_message("ERC20 transfer impossible: check your ETH balance") {
        //(sender: felt, recipient: felt, amount: Uint256）
        IERC20.transferFrom(erc20, caller, _admin_address, new_price);
    }
    return ();
}
...
func mint_domain{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    stark_id_update.emit(1, new (domain), token_id, expiry);
    domain_to_addr_update.emit(1, new (domain), target_address);
    let (contract) = starkid_contract.read();
    StarkId.set_verifier_data(contract, token_id, 'name', hashed_domain);
    if (resolver != 0) {
...
func assert_purchase_is_possible{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    alloc_locals;
    // Verify that the stark.id doesn't already manage a domain
    let (naming_contract) = get_contract_address();
    let (current_timestamp) = get_block_timestamp();
    assert_empty_stark_id(token_id, current_timestamp, naming_contract);
...
func assert_control_domain{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // check ownership
    let (contract_addr) = starkid_contract.read();
...
func assert_empty_stark_id{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    stark_id, current_timestamp, naming_contract
) {
    let (contract_addr) = starkid_contract.read();
    let (sid_hashed_domain) = StarkId.get_verifier_data(
        contract_addr, stark_id, 'name', naming_contract
    );

    with_attr error_message("This stark_id already has a domain") {
...
func fetch_owner{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    contract_addr, stark_id
) -> (owner: felt) {
    if (stark_id == 0) {
        return (0,);
    }
    let (stark_id_owner) = StarkId.owner_of(contract_addr, stark_id);
    return (stark_id_owner,);
}
...
func assert_is_owner{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (domain_data) = _domain_data.read(hashed_domain);
    let stark_id = domain_data.owner;
    // shitty crashing function
    let (stark_id_owner) = fetch_owner(contract_addr, stark_id);

    // if caller owns the starknet id, he owns the domain, we return the key
    if (stark_id_owner == caller) {
